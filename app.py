...
def update_patient(pid):
    data = request.json
    conn = get_patients_db()
    conn.execute("UPDATE patients SET name=?,age=?,gender=?,phone=?,email=?,address=?,blood_group=? WHERE id=?",
                 (data['name'], data.get('age'), data.get('gender'), data.get('phone'),
                  data.get('email'), data.get('address'), data.get('blood_group'), pid))
    conn.commit()
    conn.close()
...
def download_patients_excel():
    ...
    conn = get_patients_db()
    rows = conn.execute("SELECT id,name,age,gender,phone,email,address,blood_group,created_at FROM patients ORDER BY id").fetchall()
    conn.close()
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Patients"
    headers = ['ID','Name','Age','Gender','Phone','Email','Address','Blood Group','Registered At']
...
def get_reports():
    ...
    query = '''SELECT r.*, p.name as patient_name, p.age, p.gender, p.phone as patient_phone,
...
def get_report(rid):
    conn = get_db_joined()
    row = conn.execute('''SELECT r.*, p.name as patient_name, p.age, p.gender,
...
def get_bills():
    conn = get_db_joined()
    rows = conn.execute('''
        SELECT b.*, p.name as patient_name, p.age, p.gender, p.phone as patient_phone,
               d.name as doctor_name
        FROM bills b
